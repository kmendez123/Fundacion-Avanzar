...
function DBR_wp_reset() {
	...
	if ( ! current_user_can( 'manage_options' ) ) {
		wp_send_json_error( __( 'Only an administrator can reset the database!', 'advanced-wp-reset' ) );
	}

	global $current_user, $wpdb;

	// Include the wp_install() function if it doesn't exist
	...
	// Install wordpress
	$result  = wp_install( $blogname, $current_user->user_login, $current_user->user_email, $blog_public );
	$user_id = $result['user_id'];

	// Restore user password
	$query = $wpdb->prepare( "UPDATE $wpdb->users SET user_pass = %s, user_activation_key = '' WHERE ID = %d", $old_password, $user_id );
	$wpdb->query( $query );
	$current_user->user_pass = $old_password;

	// Remove the default password nag
...
function DBR_prepare_explanations_for_custom_reset_items(){
	...
	$all_explanations_array['uploads-files'] 	= sprintf( __( "All media uploads inside %s directory will be deleted! This includes images, videos, music, documents, sub-folders, etc.", 'advanced-wp-reset' ), $uploads_dir_path );
